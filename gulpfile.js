...
gulp.task('screenshot:rev', function(){
  return gulp.src('public/specs/screenshots/*.png')
    .pipe($.rev())
...
gulp.task('screenshot:revreplace', ['screenshot:rev'], function(){
  return gulp.src([dirs.screenshots + '/rev-manifest.json', 'public/specs/index.html'])
    .pipe($.revCollector({
      replaceReved: true,
      dirReplacements: {
        '/specs/screenshots': '/build/screenshots'
      }
    }))
